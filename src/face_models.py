...
class ResNetTransfer(nn.Module):
    ...
    def __init__(self, num_classes: int = 18, freeze_backbone: bool = True):
        super().__init__()
    ...
    def forward(self, x):
        # Extract features up to before final FC layer
...
class ArcFaceNet(nn.Module):
    ...
    def __init__(self, num_classes=18):
        ...
        # Add BatchNorm after the embedding layer
        self.embedding = nn.Sequential(
            nn.Linear(512, 512),  # embedding dimension
            nn.BatchNorm1d(512)
        )
    ...
    def forward(self, x, labels=None):
        ...
        embeddings = F.normalize(embeddings, p=2, dim=1)  # Ensure normalization
        
        if self.training:
            if labels is None:
                # Using a fallback for experiment summary generation
                # This prevents errors when generating summaries
                import logging
                logging.getLogger(__name__).warning("WARNING: ArcFace model called in training mode without labels. Switching to eval mode for forward pass.")
                self.eval()
                result = embeddings
                self.train()
                return result
            # Return logits for loss calculation outside
...
class HybridNet(nn.Module):
    ...
    def __init__(self, num_classes=18):
        ...
        # Transformer blocks - add residual connections between blocks
        self.transformer1 = TransformerBlock(self.fdim)
        self.transformer2 = TransformerBlock(self.fdim)
    ...
    def forward(self, x):
        ...
        # Apply transformer with proper dimension ordering for layer normalization
        # First transformer block
        feats_temp = feats.permute(1, 0, 2)  # seq, batch, dim -> batch, seq, dim
        feats_temp = self.transformer1(feats_temp.permute(1, 0, 2))  # back to seq, batch, dim
        feats = feats + feats_temp  # Residual connection
        
        # Second transformer block
        feats_temp = feats.permute(1, 0, 2)  # seq, batch, dim -> batch, seq, dim
        feats_temp = self.transformer2(feats_temp.permute(1, 0, 2))  # back to seq, batch, dim
        feats = feats + feats_temp  # Residual connection
    ...
    def get_embedding(self, x):
        ...
        # Apply transformer with proper dimension ordering
        feats_temp = feats.permute(1, 0, 2)
        feats_temp = self.transformer1(feats_temp.permute(1, 0, 2))
        feats = feats + feats_temp
        
        feats_temp = feats.permute(1, 0, 2)
        feats_temp = self.transformer2(feats_temp.permute(1, 0, 2))
        feats = feats + feats_temp
...
def get_model(model_type: str, num_classes: int = 18, input_size: Tuple[int, int] = (224, 224)) -> nn.Module:
    if model_type == 'baseline':
        ...
        return ResNetTransfer(num_classes=num_classes, freeze_backbone=True)
    ...
    def forward(self, x):
        ...
        for model in self.models:
            ...
            if isinstance(model, ArcFaceNet):
                ...
                # This is just a placeholder - you would need proper inference logic 
                continue
            else:
